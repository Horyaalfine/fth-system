...
@api_bp.route('/api/debug/student-plan', methods=['GET'])
@require_auth
def debug_student_plan():
    """Debug: check a specific student's agreed slots and schedule records.
    Params: admission_id (e.g. H180b), branch_id, date (YYYY-MM-DD)"""
    from datetime import datetime
    admission_id = request.args.get('admission_id','').strip()
    branch_id = request.args.get('branch_id', type=int) or branch_scope()
    ...
    try:
        # Find student
        cur.execute("SELECT id, name, admission_id, status, branch_id FROM students WHERE admission_id ILIKE %s", (admission_id,))
        student = row(cur)
        if not student:
            return jsonify({'error': f'Student {admission_id} not found'})
        sid = student['id']
        # Agreed slots
        cur.execute("""
            SELECT sas.id, sas.branch_schedule_id, sas.subject, sas.effective_from,
                   bs.day_of_week, bs.slot_start, bs.slot_end, bs.status as sched_status,
                   bs.effective_from as sched_eff_from, bs.effective_to as sched_eff_to,
                   bs.branch_id as sched_branch_id
            FROM student_agreed_slots sas
            JOIN branch_schedule bs ON bs.id = sas.branch_schedule_id
            WHERE sas.student_id = %s
        """, (sid,))
        agreed = cur.fetchall()
        # Timetable entries
        cur.execute("SELECT * FROM student_timetable WHERE student_id=%s", (sid,))
        timetable = cur.fetchall()
        # Would they appear in base_rows for this date?
        cur.execute("""
            WITH sched AS (
                SELECT id, slot_start, slot_end,
                       ROW_NUMBER() OVER (PARTITION BY branch_id ORDER BY slot_start) AS session_num
            ...
            SELECT sc.id AS sched_id, sc.slot_start, sc.slot_end, sc.session_num,
                   sas.subject AS agreed_subject
            FROM student_agreed_slots sas
            JOIN sched sc ON sc.id = sas.branch_schedule_id
            WHERE sas.student_id = %s
        """, (day_of_week, sid))
        in_base_rows = cur.fetchall()
        cur.close(); conn.close()
        return jsonify({
            'student': dict(student),
            'agreed_slots': [dict(r) for r in agreed],
            'timetable': [dict(r) for r in timetable],
            'would_appear_in_base_rows': [dict(r) for r in in_base_rows],
            'query_day_of_week': day_of_week,
            'query_day_type': day_type,
        })
    except Exception as e:
        cur.close(); conn.close()
        return jsonify({'error': str(e)}), 400
