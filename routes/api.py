...
def save_attendance():
    ...
    if present_ids:
        ...
        # Match by date+slot of ANY session on the same day (not just exact session ID)
        # so students assigned to a different table than their catchup_session_id still complete
        cur.execute(
